...
| 想抓评论/阅读/点赞/页面风格 | 场景 2C：常驻代理增强 | `wechat_downloader.py proxy-enhancer-start` |
...
默认使用最简单链路：启动 `23344` 常驻增强代理，并把系统 HTTP/HTTPS 代理切到 `23344`。`--upstream-proxy auto` 会自动判断当前系统代理：有上游代理就把 `23344` 出口串到上游；没有上游代理就直连；如果当前系统代理已经是 `23344`，不会把 `23344` 当成自己的上游。
...
```bash
python3 {baseDir}/scripts/wechat_downloader.py proxy-enhancer-session-start --port 23344 --upstream-proxy auto --yes
```

如果修改了代理增强代码或需要重载 `23344`，只能使用安全重载：

```bash
python3 {baseDir}/scripts/wechat_downloader.py proxy-enhancer-restart --port 23344 --upstream-proxy auto --yes
```

不要在系统代理指向 `23344` 时直接 `kill`、`proxy-enhancer-stop` 或 `stop && start`；这会让系统代理短时间指向死端口。安全重载会先临时切到上游代理，再重启 `23344`，最后切回 `23344`。

启动后常见链路：

```text
系统/微信 -> 127.0.0.1:23344 -> 当前上游代理（通常是 127.0.0.1:10808）-> 外网
```

用以下命令检查本机路由事实和最近是否有微信文章请求进入 `23344`：

```bash
python3 {baseDir}/scripts/wechat_downloader.py proxy-enhancer-route-help --port 23344
python3 {baseDir}/scripts/wechat_downloader.py proxy-enhancer-check-ingress --port 23344 --minutes 10
...
该命令只恢复系统代理，不停止 `23344` 常驻服务。

用户之后正常打开公众号文章。页面加载完成后应自动出现注入按钮：顶部元信息行一个，评论区标题旁一个，点击任意一个都会保存当前快照。
