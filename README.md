...
| 阅读时发现好文章或高价值评论 | 边浏览边保存正文、已加载评论和页面互动数据 | 微信收藏 | 否 | 是 |
...
Skill 会开启微信收藏，并重置微信的文章 WebView 进程以加载最新注入代码；微信主进程和登录状态不会退出。在微信文章页等待正文和评论加载完成，然后点击注入的 `收藏到本地` 按钮。你可以继续浏览并收藏多篇，不需要每篇都回到对话操作。成功注入的文章页会禁用本地缓存，后续重载不会继续复用旧按钮代码。

打开同一公众号任意一篇文章后，已创建的互动任务会在短时凭证有效窗口内继续补充互动指标和**精选评论**；不承诺全量评论或完整回复树。常驻服务尚未启用，当前仍需显式启动和结束微信收藏会话。
